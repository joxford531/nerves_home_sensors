defmodule WeatherSensor.BmpSensor do
  ...
  def raw_temp(ref) do
    I2C.write(ref, Application.get_env(:weather_sensor, :bmp_pin), <<0xf4, 0x2e>>)
    Process.sleep(5)
    {:ok, <<value::big-unsigned-size(16)>>} = I2C.write_read(ref, Application.get_env(:weather_sensor, :bmp_pin), <<0xf6>>, 2)
    value
  end

  def raw_pressure(ref) do
    I2C.write(ref, Application.get_env(:weather_sensor, :bmp_pin), <<0xf4, (0x34 + (@oss_mode <<< 6))>>)
    Process.sleep(26)
    {:ok, <<msb::big-unsigned-size(8)>>} = I2C.write_read(ref, Application.get_env(:weather_sensor, :bmp_pin), <<0xf6>>, 1)
    {:ok, <<lsb::big-unsigned-size(8)>>} = I2C.write_read(ref, Application.get_env(:weather_sensor, :bmp_pin), <<(0xf6 + 1)>>, 1)
    {:ok, <<xlsb::big-unsigned-size(8)>>} = I2C.write_read(ref, Application.get_env(:weather_sensor, :bmp_pin), <<(0xf6 + 2)>>, 1)
  ...
  def calibration(ref) do
    ...
    I2C.write_read(ref, Application.get_env(:weather_sensor, :bmp_pin), <<0xAA>>, 22)
